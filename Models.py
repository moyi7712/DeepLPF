...
class CubicFilter(Filters):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        factor = tf.reshape(factor, (1, 1, self.factor_num, 1))

        # operator
        kernel_R, kernel_G, kernel_B = tf.split(factor, 3, axis=2)
        axis_tf = self.get_axis(w, h)
        cubic_R, cubic_G, cubic_B = self.get_cubic20_mask(img, axis_tf)

        mask_R = tf.nn.conv2d(cubic_R, kernel_R, strides=1, padding='VALID')
        mask_G = tf.nn.conv2d(cubic_G, kernel_G, strides=1, padding='VALID')
        mask_B = tf.nn.conv2d(cubic_B, kernel_B, strides=1, padding='VALID')
...
class GraduatedFilter(Filters):
    ...
    def get_channel_mask(self, s, ginv, d1, d2, top_line):
        f, grad = tf.cond(ginv == 0, lambda: ((s - 1) / (2 * d1) + (s - 1) / (2 * d2), s),
                          lambda: ((s - 1) / (2 * d1) + (1 - s) / (2 * d2), 1))
        clip_min, clip_max = tf.cond(s >= 1, lambda: (1, self.max_scale), lambda: (0, 1))
        mask = tf.clip_by_value(f + grad * top_line, clip_min, clip_max)
        mask = tf.clip_by_value(mask, 0, self.max_scale)
        return mask

    def get_Graguated_mask(self, factor, axis_tf):
        sr, sg, sb, m, c, o1, o2, g = tf.split(factor, 8)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        factor_R, factor_G, factor_B = tf.split(factor, 3)
...
class EllipticalFilter(Filters):
    ...
    def get_channel_mask(self, cond, sc, radius, m):
        ...
        # mask = tf.cond(cond < tf.ones_like(cond), lambda: (m * (1 - sc) / radius + sc)*tf.ones_like(cond), lambda: tf.ones_like(cond))
        mask = tf.clip_by_value(mask, 0, self.max_scale)
        return mask

    def get_Elliptical_mask(self, factor, axis_tf):
        sr, sg, sb, h, k, theta, a, b = tf.split(factor, 8)
        axis_x, axis_y = tf.split(axis_tf, 2, axis=3)
